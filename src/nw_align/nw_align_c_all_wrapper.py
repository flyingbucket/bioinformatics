...
lib.nw_align_c_full_omp.argtypes = [
    ctypes.c_int,  # num_records
    ctypes.c_void_p,  # full_seq (uint8_t*)
    ctypes.c_void_p,  # seq_len (int32_t*)
    ctypes.c_void_p,  # seq_offset (int32_t*)
...
lib.nw_align_c_full_omp.restype = ctypes.POINTER(Result)
...
def nw_align_c_all_omp_records(processed_records):
    num_records = len(processed_records)
    if num_records < 2:
        return []
    ...
    res_ptr = lib.nw_align_c_full_omp(
        num_records,
        full_seq.ctypes.data,
        seq_len.ctypes.data,
        seq_offset.ctypes.data,
